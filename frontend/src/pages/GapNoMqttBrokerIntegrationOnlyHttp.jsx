...
// Auto-generated page for: No MQTT broker integration (only HTTP ingestion)

export default function GapNoMqttBrokerIntegrationOnlyHttp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-mqtt-broker-integration-only-http', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No MQTT broker integration (only HTTP ingestion)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No MQTT broker integration (only HTTP ingestion)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No MQTT broker integration (only HTTP ingestion)'}
        </button>
      </form>
