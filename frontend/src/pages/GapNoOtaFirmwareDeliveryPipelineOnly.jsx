...
// Auto-generated page for: No OTA firmware delivery pipeline (only metadata CRUD)

export default function GapNoOtaFirmwareDeliveryPipelineOnly() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-ota-firmware-delivery-pipeline-only', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No OTA firmware delivery pipeline (only metadata CRUD)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No OTA firmware delivery pipeline (only metadata CRUD)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No OTA firmware delivery pipeline (only metadata CRUD)'}
        </button>
      </form>
